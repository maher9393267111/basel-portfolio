          ...
          return (
            <div key={item.id} className="pt-[62.5%] relative arabic">
              <div className="absolute inset-0 w-full h-full group rounded-md overflow-hidden">
                ...
                />
                <div className="absolute bottom-0 left-0 w-full h-[54px] sm:h-[64px] group-hover:h-full bg-gradient-to-r from-secondary group-hover:bg-gradient-to-b group-hover:from-primary group-hover:via-primary group-hover:rounded-md rounded-b-md transition-background duration-500 ease-in-out  bg-primary text-center transition-height duration-500 ease-in-out overflow-hidden">
                  <p
                    className={`absolute w-fit min-w-[18ch] top-1/2 left- px-3 transform -translate-y-1/2 group-hover:top-10 sm:group-hover:top-14 text-whit text-xl shimmer font-semibold group-hover:font-bold text-md group-hover:text-[1.3rem] sm:group-hover:text-2xl transition-all duration-[500ms] ease-in-out`}
                  >
                    {item.title}
                  </p>
                  <p className="absolute transform invisible opacity-0 scale-0 group-hover:visible group-hover:opacity-100 group-hover:scale-100 top-[4.5rem] sm:top-24 text-center text-md px-4 md:px-8 text-base text-white itali arabic font-norma line-clamp-3 transition-all duration-500 delay-300 ease-in-out">
                    {item.desc}
                  </p>
                </div>
              </div>
